...
public class OrderServiceImpl implements OrderService {
	...
	public MyOrder addOrder(MyOrder order, Integer customerId, Integer addressId) {
		...
		for (Products p : products) {
			...
			for (Products prd : pr) {
				if (prd.getProductId().equals(p.getProductId())) {
					products2.add(prd);
				}
		...
		List<Address> customerAddr = customer.getAddresses();
		int count = 0;
		...
		if (count == customerAddr.size())
			throw new AddressNotFound("Address not found with the customer Id" + customerId);
	...
	@Override
	public List<Products> viewOrderByCustomerId(Integer customerId) {
	    List<Products> customerProducts = new ArrayList<>();
	    List<MyOrder> allOrders = orderdao.findAll();
	    ...
	    if (customerProducts.isEmpty()) {
	        throw new CustomerNotFoundException("No products found for the customer ID: " + customerId);
	    }
	...
	public List<MyOrder> findOrderByUserName(String FirstName, String LastName, String mobileNo) {
		...
		for (MyOrder order : allOrder) {
			if (order.getCustomer().getFirstName().equals(FirstName)
					...
					&& order.getCustomer().getMobileNumber().equals(mobileNo)) {
...
//		System.out.println(findAllByName);
	 ...
	 * List<MyOrder>allOrder=orderdao.findAll();
	 * 
	 * Integer order1=null; for(MyOrder order:allOrder) {
	 * if(order.getCustomer().getCustomerId()==custiomerId)//30=30 {
	 * //orderdao.delete(order); order1=order.getOrderid();
	 * System.out.println(order1);
	 * 
	 * // User user= userDao.findByMobile(order.getCustomer().getMobileNumber()); //
	 * String logedinOrNot = currUserSessDao.findByUserId(user.getUserId()); //
	 * if(logedinOrNot==null) { // throw new
	 * CustomerNotFoundException("Customer not logged in"); // } //
	 * order.setCustomer(null); // order.setProductlist(null); //
	 * orderdao.save(order); // System.out.println(order);
	 * orderdao.deleteById(order1);
	 * 
	 * return "order canceled"; } }
	 * 
	 * //throw new CustomerNotFoundException("Order not found");
	 * 
	 * for (MyOrder order : allOrder) { //Customer customer = order.getCustomer();
	 * if (order.getCustomer().getCustomerId() == custiomerId) {
	 * orderdao.delete(order); // Rest of the code... } } return "order canceled"; }
	 * 
	 * 
	 * @Override public String removeOrder(Integer customerId) { List<MyOrder>
	 * allOrders = orderdao.findAll();
	 * 
	 * for (MyOrder order : allOrders) { Customer customer = order.getCustomer(); if
	 * (customer != null && customer.getCustomerId() == customerId) {
	 * orderdao.delete(order); // Delete the order object from the database return
	 * "Order canceled"; } }
	 * 
	 * throw new CustomerNotFoundException("Order not found"); }
	 */
	...
	public MyOrder addorderFromCart(Integer customerId) {
		...
		for (Cart newCart : allCartDetails) {
			if (newCart.getCustomerlist().getCustomerId() == customerId)// 30==30
			{
		...
		for (Cart newCart : allCartDetails) {
			if (newCart.getCustomerlist().getCustomerId() == customerId) {
				cartId=newCart.getCartItemId();
				address = newCart.getCustomerlist().getAddresses().get(0);
				break;
			}
		}
		if (address == null) {
			throw new AddressNotFound("Address not found for customer with Id " + customerId);
		}
	...
	public String addOrdernew(Integer customerId, Integer addressId, Integer productId, MyOrder order) {
		 System.out.println("Oder new");
		 
		 
		Customer customer=customerService.viewCustomer(customerId);
		Address address=addressService.viewAddress(addressId);
		Products product=productService.getProductsFromCatalogById(productId);
